...
export const validatePassword = (password: string, res: Response) => {
    ...
    if (!hasUppercase(password)) {
        return res.status(400).json({ error: 'The password must contain at least one uppercase letter.' });
    }
    if (!hasLowercase(password)) {
        return res.status(400).json({ error: 'The password must contain at least one lowercase letter.' });
    }
    if (!hasDigit(password)) {
        return res.status(400).json({ error: 'The password must contain at least one digit.' });
    }
    if (!hasSpecialCharacter(password)) {
        return res.status(400).json({ error: 'The password must contain at least one special character (@$!%*?&).' });
    }
};
